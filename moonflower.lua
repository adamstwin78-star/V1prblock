-- moonflower | maintained by V1PR | original by Glovsaken
print("moonflower loaded")
...
secInterface:Toggle({
    Title = "Spoof Usernames", Type = "Checkbox", Flag = "spoofUsernames", Default = spoofActive,
...
do
    ...
    local function elliotAimAt(tgt)
        ...
        if vel.Magnitude > elliotVelThresh then
            predPos = predPos + (vel.Unit * elliotPredDist)
        end
    ...
    elliotSection:Toggle({ Title="Enable Elliot Aimbot", Type="Checkbox", Default=false, Callback=function(v)
        ...
        if v then
            elliotConnection = svc.Run.RenderStepped:Connect(function()
                ...
                if elliotShowArc then elliotUpdateArc(elliotFindTarget()) end
...
do
    ...
    local function bsPerformDashStabTech(killerModel)
        ...
        local bsCam = svc.WS.CurrentCamera
        ...
        -- Step 3.5: disable collision for the lunge window so you pass through the killer
        bsLungeNoclip()

        -- Step 4: 180° camera + character flip to match killer's LookVector.
        --         This is the manual camera flick the tech requires — done automatically.
        --         Camera spins 180° so the client view matches the new facing direction.
        ...
        if khrp2 and hrp2 then
            ...
            local nudgedPos=Vector3.new(
                hrp2.Position.X+look.X*BS_DEPTH_OFFSET,
                hrp2.Position.Y,
                hrp2.Position.Z+look.Z*BS_DEPTH_OFFSET
            )
            -- Flip HRP to face same direction as killer
            hrp2.CFrame=CFrame.lookAt(nudgedPos,nudgedPos+look,Vector3.new(0,1,0))
            -- Flip camera 180°: switch to Scriptable so the game engine
            -- doesn't immediately snap it back, set the CFrame, then restore.
            local prevCamType=bsCam.CameraType
            bsCam.CameraType=Enum.CameraType.Scriptable
            local camOffset=bsCam.CFrame.Position-hrp2.Position
            local newCamPos=hrp2.Position+Vector3.new(camOffset.X,camOffset.Y,camOffset.Z)
            bsCam.CFrame=CFrame.lookAt(newCamPos,newCamPos+look,Vector3.new(0,1,0))
        end

        -- Step 5: hold rotation + camera locked for the hitbox window, then restore
        local holdStart=os.clock(); local holdConn
        holdConn=svc.Run.Heartbeat:Connect(function()
            if not bsRunning or os.clock()-holdStart>=BS_LUNGE_HOLD_DURATION then
                holdConn:Disconnect()
                bsSetAutoRotate(true)
                -- Restore camera type so player regains normal control
                pcall(function() bsCam.CameraType=Enum.CameraType.Custom end)
                return
            end
            local khrp3=killerModel:FindFirstChild("HumanoidRootPart")
            local char3=bsGetChar(); local hrp3=char3 and char3:FindFirstChild("HumanoidRootPart")
            if khrp3 and hrp3 then
                local look=khrp3.CFrame.LookVector
                hrp3.CFrame=CFrame.lookAt(hrp3.Position,hrp3.Position+look,Vector3.new(0,1,0))
                -- Keep camera locked to the flicked direction every frame
                local camOff=bsCam.CFrame.Position-hrp3.Position
                local cp=hrp3.Position+Vector3.new(camOff.X,camOff.Y,camOff.Z)
                bsCam.CFrame=CFrame.lookAt(cp,cp+look,Vector3.new(0,1,0))
            end
        end)
    end
    ...
    local function bsSetNoclip(state)
        ...
        -- Persistent noclip: just disable CanCollide on all character parts every Stepped.
        -- This lets you walk through killers during the lunge. Floor/walls still work
        -- because Roblox terrain uses a separate collision path that ignores CanCollide
        -- on R15 character meshes in most executor environments.
    ...
    -- Called by bsPerformDashStabTech to disable collision only during the lunge window
    local function bsLungeNoclip()
        if not bsNoclipEnabled then
            -- temporarily disable for the lunge window even if toggle is off
            local char = lp.Character; if not char then return end
            for _, part in ipairs(char:GetDescendants()) do
                if part:IsA("BasePart") then part.CanCollide = false end
            end
            task.delay(BS_DASH_FLIP_DELAY + BS_LUNGE_HOLD_DURATION + 0.1, bsRevertCanCollide)
        end
    end
    ...
    local function bsStartCrouchStab()
        ...
        if not btn then
            warn("[moonflower] CrouchStab: Crouch button not found")
            return
        end
        bsCrouchStabConn = btn.MouseButton1Down:Connect(function()
            ...
            task.spawn(function()
                task.wait(bsCrouchStabDelay) -- let server register Crouching attribute first
                bsTryActivateButton(bsGetDaggerButton())
            end)
        end)
    end
    ...
    -- Hook on load and re-hook on respawn
    task.spawn(function()
        task.wait(2)
        bsStartCrouchStab()
    end)
    lp.CharacterAdded:Connect(function()
        task.wait(2)
        if bsCrouchStabEnabled then bsStartCrouchStab() end
    end)
    ...
    bsSection:Toggle({ Title="Crouch Insta-Stab", Type="Checkbox", Default=false,
        Desc="Tap crouch → dagger fires after delay",
        Callback=function(v)
            bsCrouchStabEnabled=v
            if v then bsStartCrouchStab() else bsStopCrouchStab() end
        end })

    bsSection:Slider({ Title="Crouch Stab Delay (s)", Step=0.01,
        Desc="Time after crouch before dagger fires. 0.10 = default.",
        Value={Min=0.00, Max=0.50, Default=0.10},
        Callback=function(v) bsCrouchStabDelay=v end })
