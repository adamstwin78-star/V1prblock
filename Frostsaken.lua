-- Frostsaken | maintained by V1PR | original by Glovsaken
-- REWRITE: Rayfield GUI | Custom theme override | ESP color pickers
...
------------------------------------------------------------------------
-- Custom Theme Color Helper
-- Reads saved RGB from config and applies it as a full Rayfield override
------------------------------------------------------------------------
local customThemeR = cfg.get("themeR", 100)
local customThemeG = cfg.get("themeG",  60)
local customThemeB = cfg.get("themeB", 220)

local function applyCustomTheme(r, g, b)
    -- Convert 0-255 → Rayfield Theme table format
    local function pct(v) return math.clamp(v / 255, 0, 1) end
    ...
    local lighter = Color3.fromRGB(
        math.min(255, math.floor(r * 1.3 + 30)),
        math.min(255, math.floor(g * 1.3 + 30)),
        math.min(255, math.floor(b * 1.3 + 30))
    )
    pcall(function()
        Rayfield:SetTheme({
            Default    = Color3.fromRGB(20,  20,  30),
            White      = Color3.fromRGB(245, 245, 255),
            Darker     = Color3.fromRGB(14,  14,  22),
            Dark       = Color3.fromRGB(18,  18,  28),
            Light      = Color3.fromRGB(30,  30,  45),
            Highlight  = accent,
            Accent     = accent,
            Secondary  = darker,
            Muted      = lighter,
...
applyCustomTheme(customThemeR, customThemeG, customThemeB)
...
local win = Rayfield:CreateWindow({
    ...
    LoadingSubtitle  = "by V1PR / Glovsaken",
    ConfigurationSaving = {
        Enabled = true,
...
local secAppearance = tabSettings:CreateSection("Appearance")

------------------------------------------------------------------------
-- Custom Theme Color Picker (replaces theme dropdown)
-- One picker controls the FULL Rayfield GUI accent colour
------------------------------------------------------------------------

tabSettings:CreateColorPicker({
    Name    = "GUI Theme Color",
    Color   = Color3.fromRGB(customThemeR, customThemeG, customThemeB),
    Flag    = "guiThemeColor",
...
tabSettings:CreateParagraph({ Title = "How it works", Content = "Repeatedly fires a SetDevice network call telling the server you are on a different platform (PC, Mobile, or Console). Some killers and abilities behave differently depending on platform." })
...
tabSettings:CreateToggle({ Name = "Enable Spoofer", CurrentValue = platEnabled, Flag = "platEnabled",
    Callback = function(on) platEnabled = on; cfg.set("platEnabled", on); if on then platStart() else platStop() end end })
tabSettings:CreateDropdown({ Name = "Device", Options = { "PC", "Mobile", "Console" }, CurrentOption = platDevice,
    Flag = "platDevice",
...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: VISUAL (ESP) — event-driven, zero polling
------------------------------------------------------------------------
------------------------------------------------------------------------
...
tabVisual:CreateParagraph({ Title = "How it works", Content = "Attaches a Highlight and a BillboardGui label to each tracked object. Fully event-driven — ChildAdded/ChildRemoved connections update ESP instantly when players spawn or objects appear, with zero polling loops. Use the colour pickers below to customise each category." })
...
-- Custom ESP colours (saved per-category)
...
local espColors = {
    killer    = loadColor("espColKiller",   255,   0,   0),
    survivor  = loadColor("espColSurvivor", 255, 255,   0),
    generator = loadColor("espColGen",      255, 105, 180),
    item      = loadColor("espColItem",       0, 230, 230),
    building  = loadColor("espColBuilding", 255,  80,   0),
}
...
local function espItemColor(name)
    local n = name:lower()
    if n:find("medkit")    then return espColors.item end
    if n:find("bloxycola") then return espColors.item end
    ...
    for _, obj in ipairs(ig:GetDescendants()) do
        if obj.Name == "BloxyCola" or obj.Name == "Medkit" then
            if not espItemHeld(obj) then
                if on then espAttach(obj, "esp_i", espColors.item, false)
...
local function espDoBuildings(on)
    ...
    for _, obj in ipairs(ig:GetChildren()) do
        if obj.Name == "BuildermanSentry" or obj.Name == "SubspaceTripmine" or obj.Name == "BuildermanDispenser" then
            if on then espAttach(obj, "esp_b", espColors.building, false)
    ...
    table.insert(esp.mapConns, ig.DescendantAdded:Connect(function(obj)
        ...
        task.defer(function()
            if obj and obj.Parent and not espItemHeld(obj) then
                espAttach(obj, "esp_i", espColors.item, false)
            end
        end)
    end))
...
tabVisual:CreateToggle({ Name="Items",      CurrentValue=esp.items,      Flag="espItems",      Callback=function(on) esp.items=on;      cfg.set("espItems",on);      espDoItems(on)      end })
tabVisual:CreateToggle({ Name="Buildings",  CurrentValue=esp.buildings,  Flag="espBuildings",  Callback=function(on) esp.buildings=on;  cfg.set("espBuildings",on);  espDoBuildings(on)  end })
...
tabVisual:CreateColorPicker({
    Name    = "Item Color",
    Color   = espColors.item,
    Flag    = "espColItem",
    Callback = function(col)
        espColors.item = col; saveColor("espColItem", col)
...
tabVisual:CreateParagraph({ Title = "How it works", Content = "Tracks c00lkidd's pizza bots (orange), 1x1x1x1 zombies (green), and John Doe's shadow puddles (black disc with red glow). Each uses AncestryChanged to self-clean when the object is removed." })
...
local function addPuddleHighlight(part, color, tag, label)
    ...
    task.defer(function()
        ...
        local bb = Instance.new("BillboardGui")
        bb.Name=tag.."_BB"; bb.Adornee=holder
        bb.Size=UDim2.new(0,150,0,22); bb.StudsOffset=Vector3.new(0,1.5,0)
        bb.AlwaysOnTop=true; bb.Parent=holder
...
tabMusic:CreateParagraph({ Title = "How it works", Content = "Detects when Last Man Standing starts by counting alive survivors or checking the LMS sound. Swaps the game's LastSurvivor sound ID to your chosen track and plays it. Tracks are downloaded once and cached to your device so they load instantly on future rounds." })
...
tabSurSen:CreateParagraph({ Title = "How it works", Content = "Loads external sentinel scripts for specific killers. Guest1337 loads its own dedicated script from Pastebin." })

tabSurSen:CreateButton({ Name="Guest1337",
...
do
    ...
    tabSurSen:CreateParagraph({ Title = "How it works", Content = "Aims your HRP toward the nearest killer using a BodyGyro. Velocity mode predicts where the killer will be based on their current speed. Ping mode accounts for your network latency. Hold-to-Aim lets you hold a key (default Q) to activate." })
    ...
    tabSurSen:CreateButton({ Name="Ping Aim Preset", Callback=function()
        chanceAimEnabled=true; chancePredMode="Ping"; chancePredValue=1.5
        Rayfield:Notify({ Title="Preset Applied", Content="Ping mode, value 1.5", Image=4483362458, Duration=3 })
    end })
    tabSurSen:CreateButton({ Name="Owner's Setting Opinion", Callback=function()
        Rayfield:Notify({ Title="Owner's Opinion", Content="0.5 works best for 90-120 ping / Same for ping 1.5 / If high ping use 0.4 velocity / Else 1.3 ping", Image=4483362458, Duration=10 })
    end })
    tabSurSen:CreateButton({ Name="Tester's Setting Opinion", Callback=function()
        Rayfield:Notify({ Title="Tester's Opinion", Content="60 ping → 0.1-0.2 / 120 ping → ~0.5 / 200-300 ping → don't use", Image=4483362458, Duration=10 })
    end })
    tabSurSen:CreateToggle({ Name="Custom Shoot Anim", CurrentValue=false, Callback=function(v) chanceCustomAnim=v end })
    tabSurSen:CreateInput({ Name="Animation ID", PlaceholderText="ID here", RemoveTextAfterFocusLost=false, Callback=function(v) chanceCustomAnimID=v end })
end
...
do
    ...
    local function bsGetOrCreateRadiusPart(killer)
        ...
        part.Name="FrostsakenRadius"; part.Anchored=true; part.CanCollide=false
    ...
    local function bsStartCrouchStab()
        ...
        if not btn then warn("[Frostsaken] CrouchStab: Crouch button not found"); return end
    ...
    tabSurSen:CreateParagraph({ Title = "How it works", Content = "Auto Backstab: detects when a killer is behind you within range and in the correct angle cone, then rotates your HRP to face them and optionally fires your dagger. Dash-Stab Tech: when you are crouching and a killer lunges at you, it snaps you to face their lunge direction." })
    ...
    tabSurSen:CreateToggle({ Name="Crouch Insta-Stab", CurrentValue=false,
        Callback=function(v) bsCrouchStabEnabled=v; if v then bsStartCrouchStab() else bsStopCrouchStab() end end })
    tabSurSen:CreateSlider({ Name="Crouch Stab Delay (s)", Range={0.00,0.50}, Increment=0.01, CurrentValue=0.10, Callback=function(v) bsCrouchStabDelay=v end })
    tabSurSen:CreateDropdown({ Name="Mode", Options={"Auto Backstab","Dash-Stab Tech"}, CurrentOption="Auto Backstab",
        Callback=function(v) bsMode=v end })
    bsDetectionRangeSlider=tabSurSen:CreateSlider({ Name="Detection Range", Range={1,32}, Increment=1, CurrentValue=BS_DEFAULT_PROXIMITY,
        Callback=function(v) if not bsSuppressCallback then bsBaseProximity=v end end })
    tabSurSen:CreateSlider({ Name="Aim Snap Delay (s)", Range={0.00,0.40}, Increment=0.01, CurrentValue=BS_AIM_SNAP_DELAY,
        Callback=function(v) BS_AIM_SNAP_DELAY=v end })
    tabSurSen:CreateSlider({ Name="Flip Delay (s)", Range={0.28,0.65}, Increment=0.01, CurrentValue=BS_DASH_FLIP_DELAY, Callback=function(v) BS_DASH_FLIP_DELAY=v end })
    tabSurSen:CreateSlider({ Name="Lunge Hold (s)", Range={0.10,0.50}, Increment=0.005, CurrentValue=BS_LUNGE_HOLD_DURATION, Callback=function(v) BS_LUNGE_HOLD_DURATION=v end })
    tabSurSen:CreateSlider({ Name="Depth Offset (st)", Range={0.0,5.0}, Increment=0.25, CurrentValue=BS_DEPTH_OFFSET, Callback=function(v) BS_DEPTH_OFFSET=v end })
    tabSurSen:CreateToggle({ Name="Noclip (easier backstabs)", CurrentValue=false,
        Callback=function(v) bsSetNoclip(v) end })
    tabSurSen:CreateToggle({ Name="Show Radius Ring", CurrentValue=true,
        Callback=function(v) bsShowRadius=v; if not v then bsDestroyAllRadiusParts() end end })
    tabSurSen:CreateButton({ Name="Unload TwoTime", Callback=function()
        bsRunning=false; bsEnabled=false; bsSetNoclip(false); bsDestroyAllRadiusParts()
        Rayfield:Notify({ Title="TwoTime", Content="Unloaded.", Image=4483362458, Duration=3 })
    end})
end
...
tabVeeronica:CreateParagraph({ Title = "How it works", Content = "Monitors Veeronica's Behavior folder for Highlight instances that switch Adornee to your character — this is how the game signals a trick opportunity. The moment that happens it automatically fires the SprintingButton press so the trick executes instantly." })
...
do
    ...
    tabVeeronica:CreateParagraph({ Title = "How it works", Content = "Detects when Veeronica's charge animation (ID 117058860640843) is playing and takes over movement: forces 60 walkspeed, locks direction forward, and enables shiftlock so the camera steers you." })
end
...
do
    ...
    -- Crystal Auto-Fire UI
    tabJaneDoe:CreateSection("Crystal Auto-Fire")
    tabJaneDoe:CreateParagraph({ Title = "How it works", Content = "Patches Jane Doe's RemoteFunction so every crystal throw auto-fires on a loop. Aimbot (Silent Aim) solves the ballistic angle server-side. On PC, enable Snap Mouse to Prediction: before each shot the camera snaps to the solved angle, then immediately unlocks so you keep full mouse control between shots." })
    ...
    tabJaneDoe:CreateToggle({ Name="Snap Mouse to Prediction (PC)", CurrentValue=false,
        Callback=function(on) jd_snapMouse=on end })
    tabJaneDoe:CreateSlider({ Name="Aim Offset (Y)", Range={-5.0,5.0}, Increment=0.1, CurrentValue=jd_AIM_OFFSET,
        Callback=function(v) jd_AIM_OFFSET=v end })
    tabJaneDoe:CreateSlider({ Name="Prediction", Range={0.0,1.0}, Increment=0.01, CurrentValue=jd_PREDICTION,
        Callback=function(v) jd_PREDICTION=v end })
    tabJaneDoe:CreateSlider({ Name="Hold Duration (s)", Range={0.3,2.0}, Increment=0.1, CurrentValue=jd_HOLD_DURATION,
        Callback=function(v) jd_HOLD_DURATION=v end })

    -- Axe Lock UI
    tabJaneDoe:CreateSection("Axe Lock")
    tabJaneDoe:CreateParagraph({ Title = "How it works", Content = "Hooks your axe FireServer call. The moment you throw the axe, your character snaps and holds facing the nearest killer for the Lock Duration — giving the axe the best chance to curve into them. Your camera never moves, only your HumanoidRootPart rotates." })
    tabJaneDoe:CreateToggle({ Name="Enable Axe Lock", CurrentValue=false,
        Callback=function(on) jd_axeLockEnabled=on; if on then jd_axeStartDetection() else jd_axeStopDetection() end end })
    tabJaneDoe:CreateSlider({ Name="Prediction (s)", Range={0.0,1.0}, Increment=0.01, CurrentValue=jd_AXE_PREDICTION,
        Callback=function(v) jd_AXE_PREDICTION=v end })
    tabJaneDoe:CreateSlider({ Name="Lock Duration (s)", Range={0.5,3.0}, Increment=0.1, CurrentValue=jd_AXE_DURATION,
    ...
    tabJaneDoe:CreateButton({ Name="Unload Jane Doe", Callback=function()
        ...
        Rayfield:Notify({ Title="Jane Doe", Content="Unloaded successfully.", Image=4483362458, Duration=3 })
    end})
end

------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: NOLI (Heli-Tech + Nova Silent Aim)
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabNoli = win:CreateTab("Noli", "wind")

tabNoli:CreateSection("Heli-Tech Macro")

tabNoli:CreateParagraph({ Title = "How it works", Content = "Launches a void-rush boosted heli jump. Trigger manually with H or let the macro auto-detect your second dash. Strength controls air distance, Hold Time controls how long the boost is maintained." })

local heli = {
    enabled  = false,
    strength = cfg.get("heliStrength", 95),
    holdTime = cfg.get("heliHold",    0.32),
    isActive = false,
}
...
local function heliFireVoidRush()
    ...
    pcall(function() net:FireServerConnection("UseAbility", "REMOTE_EVENT", "VoidRush") end)
end
...
svc.Run.RenderStepped:Connect(function()
    ...
    local isSecond = (vrs == "Dashing" or vrs == "Rerush" or vrs == "SecondDash")
...
tabNoli:CreateToggle({ Name="Enable Heli-Tech Macro", CurrentValue=false,
    Callback=function(on) heli.enabled=on end })
tabNoli:CreateSlider({ Name="Launch Strength", Range={50,160}, Increment=1, CurrentValue=heli.strength,
    Callback=function(v) heli.strength=v; cfg.set("heliStrength",v) end })
tabNoli:CreateSlider({ Name="Hold Time (s)", Range={0.15,0.65}, Increment=0.01, CurrentValue=heli.holdTime,
    Callback=function(v) heli.holdTime=v; cfg.set("heliHold",v) end })
tabNoli:CreateButton({ Name="Test Heli-Tech Now (H key)", Callback=function() heliLaunch() end })

------------------------------------------------------------------------
-- NOVA SILENT AIM
------------------------------------------------------------------------
tabNoli:CreateSection("Nova Silent Aim")

tabNoli:CreateParagraph({ Title = "How it works", Content = "Patches the RemoteFunction OnClientInvoke so when the server asks 'GetCameraCF' (to determine Nova throw direction), it always returns a mathematically solved ballistic CFrame aimed at the nearest survivor with velocity prediction — completely ignoring where your camera is actually pointing." })
...
local function novaGetVelocity(hrp)
    local now = tick(); local pos = hrp.Position; local data = nova.motionData[hrp]
    if not data then nova.motionData[hrp]={lastPos=pos,lastTime=now,velocity=Vector3.zero}; return Vector3.zero end
    local dt = now - data.lastTime; if dt <= 0 then return data.velocity end
    local vel = (pos - data.lastPos) / dt
    data.lastPos = pos; data.lastTime = now; data.velocity = vel
    return vel
end
...
local function novaRemovePatch()
    ...
    nova.originalCallback = nil; nova.patched = false
end

lp.CharacterAdded:Connect(function()
    nova.motionData = setmetatable({}, {__mode="k"})
    nova.patched = false; nova.originalCallback = nil
    task.delay(1, function() if nova.enabled then novaApplyPatch() end end)
end)

tabNoli:CreateToggle({ Name="Enable Nova Silent Aim", CurrentValue=false,
    Callback=function(on)
        nova.enabled = on
        if on then novaApplyPatch() else novaRemovePatch() end
    end })
tabNoli:CreateSlider({ Name="Prediction", Range={0,2}, Increment=0.01, CurrentValue=nova.prediction,
    Callback=function(v) nova.prediction=v; cfg.set("novaPrediction",v) end })
tabNoli:CreateSlider({ Name="Aim Offset Y", Range={-5,5}, Increment=0.1, CurrentValue=nova.aimOffsetY,
